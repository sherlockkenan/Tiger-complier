...
static int match_type(Ty_ty tType, Ty_ty eType)
{
...
F_fragList SEM_transProg(A_exp exp)
{
	...
	struct expty body=transExp(Tr_outermost(), venv, tenv, NULL, exp);
  //add main()function
	Tr_procEntryExit(Tr_outermost(),body.exp,NULL);
	return Tr_getResult();
}
    ...
    case A_opExp:
    {
      ...
      switch (oper) {
        ...
        case A_neqOp:
          switch(left.ty->kind) {
            case Ty_int:
              if (match_type(right.ty, left.ty))
      ...
      return expTy(Tr_noExp(), Ty_Int());
		...
		case A_callExp:
		{
			...
			if (x && x->kind == E_funEntry) {
				...
				for (args = a->u.call.args; args && formals;
					...
					if (!match_type(arg.ty, formals->head))
		...
		case A_recordExp:
		{
			...
			for (recList = a->u.record.fields; recList;
				...
				if (!match_type(fieldTys->head->ty, e.ty))
		...
		case A_ifExp:
		{
			...
			if (a->u.iff.elsee) {
				...
				if (!match_type(then.ty, elsee.ty)) {
			...
			if (!typ) {
				...
				if (size.ty->kind != Ty_int)
					EM_error(a->u.array.size->pos,
						...
						Ty_ToString(size.ty));
				if (!match_type(typ->u.array, init.ty))
	...
	switch (d->kind) {
		case A_functionDec:
		{
			...
			for (funList = d->u.function; funList; funList = funList->tail) {
				...
				S_enter(venv, funList->head->name,E_FunEntry(funLevel, funLabel, formalTys, resultTy));
			}
			...
			for (funList = d->u.function; funList; funList = funList->tail) {
				...
				for (paramFields = funList->head->params; paramFields;
					...
					S_enter(venv, paramFields->head->name,E_VarEntry(accessList->head, paramTys->head));
				}
				struct expty e = transExp(funEntry->u.fun.level, venv, tenv,breakk, funList->head->body);
				if (!match_type(funEntry->u.fun.result, e.ty))
