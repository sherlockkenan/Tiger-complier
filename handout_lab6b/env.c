...
S_table E_base_venv(void){
  ...
  string s = String("printi");
  Ty_tyList formalsTys = Ty_TyList(Ty_Int(),NULL);
  S_enter(venv,S_Symbol(s),E_FunEntry(Tr_outermost(),S_Symbol(s),formalsTys,Ty_Void()));

  s = String("print");
  formalsTys = Ty_TyList(Ty_String(),NULL);
  S_enter(venv,S_Symbol(s),E_FunEntry(Tr_outermost(),S_Symbol(s),formalsTys,Ty_Void()));
  // 'flush()'
  s = String("flush");
  ...
  s = String("exit");
  S_enter(venv, S_Symbol(s), E_FunEntry(Tr_outermost(), Temp_newlabel(),Ty_TyList(Ty_Int(), NULL), Ty_Void()));
 return venv;
}
